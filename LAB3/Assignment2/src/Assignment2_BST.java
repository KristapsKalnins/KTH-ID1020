public class Assignment2_BST<Key extends Comparable<Key>, Value>{
    private Node root;
    private class Node {
        private Key key;
        private Value val;
    ...
    {
        Assignment2_Queue<Key> queue = new Assignment2_Queue<Key>();
        keys(root, queue, lo, hi);
        return queue;
    }


    private void keys(Node x, Assignment2_Queue<Key> queue, Key lo, Key hi)                //Infix traverse through tree to enqueue all keys in iterable queue
    {
        if (x == null) return;
